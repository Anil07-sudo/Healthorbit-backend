...
//helper function
// so this function converts array little  like input into a clean array
...
export async  function createService(req,res){
  try{
    ...
    }// if the file is present it will be uploaded to srvices folder in the cloudinary
   ...
   export async function  getServicesById(req,res) {
    ...
    try{
      ...
      const foundService = await Service.findById(id).lean();
       if(!foundService) return res.status(401).json({
        success : false,
  ...
  runValidators: true
    ...
    return res.status(200).json({
      ...
      message :" Service Updated"
          ...
          console.error("upadteService Error:",err);
    return res.status(401).json({
      success:false,
      message: "server Error"
    });
  ...
  try {
    ...
    if(existing.imagePublicId){
      try{
        ...
        console.warn("Failed to delete imagre from cloudinary:",
          err?.message || err
        );
      }
    }
