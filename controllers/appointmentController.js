...
// this function will return the finite number
...
// this function will get the user from clerk and return the user details over here
...
try{
    ...
    let session;
    try{
       session= await stripe.checkout.retrieve(session_id);
   ...
   try{
    ...
    if (body.date && body.time) {
     ...
     const updated = await Appointment.findByIdAndUpdate(id,update,{
      ...
      console.error ("upadteAppointment Error:", err);
    return res.status(500).json({success: false, message:"Server error"});
...
// // get Stats
 ...
 export const getStats = async (req,res) =>{
   try{
    ...
    console.error("getStatsAppointment error :",err);
    return res.status(500).json({success: false, message:"server error"});
   }
...
export const getAppointmentsByDoctor = async (req,res) =>{
   try{
    ...
    .populate("doctorId"," name specialization owner imageUrl image") .lean();
     ...
     console.error("getAppointmentBy doctor error :",err);
    return res.status(500).json({success: false, message:"server error"});
